...
export default function KhuyenMaiPage() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/auth/login?redirect=/khuyen-mai');
    }
  }, [isAuthenticated, isLoading, router]);

  useEffect(() => {
    if (isAuthenticated) void loadWallet();
        ...
        {vouchers.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
            ...
            <p className="mt-2 text-sm text-gray-600 max-w-md mx-auto">
              Shop sẽ tặng mã qua thông báo khi bạn đăng ký, nhận hàng, sinh nhật hoặc các dịp đặc biệt.
              Mã không hiển thị công khai — chỉ dành riêng cho bạn.
            </p>
            <Link
              href="/"
