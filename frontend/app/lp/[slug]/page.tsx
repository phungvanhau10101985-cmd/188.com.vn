...
import { buildProductJsonLd, buildProductListJsonLd, truncateDescriptionAtSentence } from '@/lib/product-seo';
...
export default async function LadipagePublicPage({ params }: Props) {
  ...
  if (isSingleProduct) {
    ...
    if (product) {
      ...
      if (seg) {
        redirect(`/products/${encodeURIComponent(seg.replace(/\//g, '-'))}`);
      }
    }
  }
