...
**Mỗi lần upgit deploy:** luôn chạy đúng một lệnh trong [`deploy-vps.cmd`](./deploy-vps.cmd) trên VPS:
...
Script tự `git pull`. Không dùng lối tắt `pm2 restart` / chỉ `git pull`.
