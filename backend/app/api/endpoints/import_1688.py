...
class Import1688CookieSettingsOut(BaseModel):
    ...
    cookie_saved_at: str | None = None
    cookie_status: str = "missing"
    cookie_warnings: List[str] = []
    cookie_expiry_known_for_all: bool = False
    cookies_all_expired: bool = False
    cookies_expired_count: int = 0
...
@router.delete("/settings/cookie", response_model=Import1688CookieSettingsOut)
def delete_import_1688_cookie_settings(
    _: AdminUser = Depends(require_privileged_admin),
):
    delete_scraper_cookies()
    upsert_scraper_cookie_env_local(
