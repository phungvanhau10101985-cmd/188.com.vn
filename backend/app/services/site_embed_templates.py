...
def expand_facebook_pixel(pixel_id: str) -> List[PlacementHtml]:
    ...
    click_id_bootstrap = (
        ...
        "function setCk(n,v){d.cookie=n+'='+encodeURIComponent(v)+max;c=d.cookie||''}"
        "function getCk(n){var a=c.split('; ');for(var i=0;i<a.length;i++){"
        "if(a[i].indexOf(n+'=')===0){var v=a[i].slice(n.length+1);try{return decodeURIComponent(v)}catch(e){return v}}}return ''}"
        "function lsGet(k){try{return localStorage.getItem(k)||''}catch(e){return ''}}"
        "function lsSet(k,v){try{localStorage.setItem(k,v)}catch(e){}}"
        "function valid(v,re){var m=re.exec(v||'');if(!m)return false;var t=Number(m[1]),n=Date.now();"
        "return Number.isSafeInteger(t)&&t<=n+300000&&t>=n-7776000000}"
        "var fbcRe=/^fb\\.\\d+\\.(\\d{13})\\.[A-Za-z0-9_-]+$/,fbpRe=/^fb\\.\\d+\\.(\\d{13})\\.\\d+$/;"
        "var clid='';try{clid=(new URLSearchParams(l.search).get('fbclid')||'').trim()}catch(e){}"
        "var ckFbc=getCk('_fbc'),storedFbc=lsGet('188_meta_fbc');"
        "var fbc=valid(ckFbc,fbcRe)?ckFbc:(valid(storedFbc,fbcRe)?storedFbc:'');"
        "if(clid){lsSet('188_meta_fbclid',clid);var suffix='.'+clid;"
        "if(!fbc||fbc.slice(-suffix.length)!==suffix)fbc='fb.1.'+Date.now()+suffix;"
        "setCk('_fbc',fbc);lsSet('188_meta_fbc',fbc)}"
        "else if(fbc){setCk('_fbc',fbc);lsSet('188_meta_fbc',fbc)}"
        "var ckFbp=getCk('_fbp'),storedFbp=lsGet('188_meta_fbp');"
        "var fbp=valid(ckFbp,fbpRe)?ckFbp:(valid(storedFbp,fbpRe)?storedFbp:'');"
        "if(!fbp)fbp='fb.1.'+Date.now()+'.'+String(Math.floor(Math.random()*1e16));"
