...
`product_info.gender` (AK) ưu tiên nếu đã điền; không có thì suy luận từ tên + danh mục
(nam → male, nữ → female, cả hai → unisex, không nhắc → để trống). Cũng dùng cho:
google_product_category, custom_label, cogs,
...
def _gender_from_raw_value(raw: str) -> str:
...
def _scrub_vietnam_place_name(text: str) -> str:
    """Tránh « Việt Nam » bị nhận nhầm là giới tính nam."""
    return re.sub(r"vi[eêệ]t\s+nam\b", " ", (text or ""), flags=re.I)


def _text_has_gender_nam(text: str) -> bool:
    t = _scrub_vietnam_place_name(text or "").strip()
    if t.endswith(" Nam"):
        return True
...
def _gender_blob_from_product(product: Product) -> str:
    parts = [
...
def infer_gender_from_name_and_categories(product: Product) -> str:
    """
    Suy luận gender feed (male / female / unisex) từ tên + breadcrumb danh mục.
    Cả nam và nữ → unisex; không nhắc giới tính → chuỗi rỗng.
    """
    blob = _gender_blob_from_product(product)
    if not blob:
        return ""
    has_nam = _text_has_gender_nam(blob)
    has_nu = _text_has_gender_nu(blob)
    if has_nam and has_nu:
        return "unisex"
    if has_nam:
        return "male"
    if has_nu:
...
def _normalized_gender(product: Product) -> str:
    pi = _product_info_dict(product)
    raw = str(pi.get("gender") or "").strip()
    if raw:
        manual = _gender_from_raw_value(raw)
        if manual:
            return manual
    return infer_gender_from_name_and_categories(product)
